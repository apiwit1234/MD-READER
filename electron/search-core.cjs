...
function buildSnippet(line, matchStart, matchLen) {
  ...
  if (chars.length <= SNIPPET_MAX) return chars.join('');
  ...
  if (to - from < SNIPPET_MAX) from = Math.max(0, to - SNIPPET_MAX);
  const left = from > 0 ? '…' : '';
...
function scanFiles(opts) {
  ...
  let totalMatches = 0;
  let truncated = false;
  if (!query) return { files: out, totalMatches: 0, truncated: false };
  ...
  outer: for (const file of sortedFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      while (from <= hay.length) {
        ...
        fileMatches.push({ line: i + 1, column: idx, lineText: buildSnippet(original, idx, query.length) });
        totalMatches++;
